...
git -C ~/.barnacle/repo fetch -q origin main
git -C ~/.barnacle/repo reset --hard -q origin/main
[ -d ~/.barnacle/venv ] || python3 -m venv ~/.barnacle/venv
~/.barnacle/venv/bin/pip install --quiet -r \
  ~/.barnacle/repo/forecast/nowcast-requirements.txt
~/.barnacle/venv/bin/pip check
~/.barnacle/venv/bin/python -m py_compile \
  ~/.barnacle/repo/forecast/nowcast.py \
  ~/.barnacle/repo/forecast/flood_forecast_daily.py
