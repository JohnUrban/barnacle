...
inv=A.inventory(); saved=json.loads((reports/'readiness-inventory-r2.json').read_text())
out['inventory']={'count':len(inv['rows']),'rows_match':canon(inv['rows'])==saved['rows']};print('inventory',out['inventory'],flush=True)
fr=F.run(inv['rows']);fs=json.loads((reports/'readiness-fidelity-r2.json').read_text());out['fidelity']={'rows':len(fr),'rows_match':canon(fr)==fs['rows'],'summary_match':canon(F.summarize(fr))==fs['summary']};print('fidelity',out['fidelity'],flush=True)
...
ctx=S.Context(inv_rows=inv['rows']);rep,pairs=S.evaluate(ctx,man['entries']);saved=json.loads((reports/'study-b-street-r2.json').read_text())
...
ev=R.per_event(pairs,man['event_peaks']);saved_ev=json.loads((reports/'study-b-events-r2.json').read_text())
...
saved_a=json.loads((reports/'study-a-advisory-r2.json').read_text());ar=saved_a['report'];levels=noaa.water_levels(noaa.load_bodies(str(ROOT/ar['outcome_manifest'])));AD.attach_outcomes(ps,levels,dt.datetime.fromisoformat(ar['evaluated_utc']));from forecast import flood_forecast_daily as ff
...
Path('/tmp/heron-r3-reproduction.json').write_text(json.dumps(out,indent=2,default=str));print('done',json.dumps(metrics),flush=True)
...
Path("/tmp/heron-r3-reproduction.json").write_text(json.dumps(out,indent=2,default=str))
