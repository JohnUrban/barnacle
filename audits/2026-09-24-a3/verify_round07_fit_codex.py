"""Independent round-07 refit: independently apply quality-aware QC.
...
# Independent quality mask; do not import the fit/evaluator classifier.
qc=flags.shape[1]==4
ok=((wf.q.eq('p') & wf.f.astype(str).str.fullmatch(r'\d+,0,0,0')) | (wf.q.eq('v') & wf.f.eq('0,0,0,0'))) & np.isfinite(pd.to_numeric(wf.v,errors='coerce'))
