...
class ModelVersionTests(unittest.TestCase):
    def test_production_constants_are_stamped_v0_10_6(self):
        self.assertEqual(ff.CURRENT_MODEL_VERSION, "v0.10.6")
    ...
    def test_current_spec_and_readmes_match_source_stamp(self):
        ...
        self.assertTrue((ROOT / "model" / "archive" / "v0.10.3.md").exists())
        self.assertFalse((ROOT / "model" / "v0.10.3.md").exists())
        self.assertTrue((ROOT / "model" / "archive" / "v0.10.4.md").exists())
        self.assertFalse((ROOT / "model" / "v0.10.4.md").exists())
        self.assertFalse((ROOT / "model" / "v0.10.5-candidate.md").exists())
        self.assertTrue((ROOT / "model" / "archive" / "v0.10.5.md").exists())
        self.assertFalse((ROOT / "model" / "v0.10.5.md").exists())
        self.assertIn(
    ...
    def test_driveway_is_a_documented_proxy_not_a_landmark(self):
        # v0.10.4 (owner decision 2026-09-20): landmarks are fixed points
        # anyone can report as under/level/over; the driveway is not one.
        self.assertFalse(any(row[0] == "driveway_central" for row in ff.LANDMARKS))
        self.assertEqual(len(ff.LANDMARKS), 18)
        playbook = (ROOT / "PLAYBOOK.md").read_text(encoding="utf-8")
        self.assertIn("PROXIES vs LANDMARKS", playbook)
        ...
        self.assertIn('const WIDGET_VERSION = "v7.29a";', widget)
