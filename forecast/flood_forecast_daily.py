...
# SEAM 1 EXTRACTED (Phase-3 split, 2026-09-02): station-local time
# lives in forecast/station_time.py; these re-exports keep every
...
def _load_surge_state(path=None):
    ...
    saved, or None. Consumed by the v0.10.6 stale-state rung."""
    try:
        with open(path or SURGE_STATE_PATH, encoding="utf-8") as f:
            rec = json.load(f)
        when = dt.datetime.fromisoformat(str(rec["observation_utc"]).replace("Z", "+00:00"))
        return (float(rec["surge_ft"]), when)
...
# ============================================================
# SEAM 2 EXTRACTED (Phase-3 split, 2026-09-02): email/site/details/
# per-tide page rendering lives in forecast/rendering.py; these
# re-exports keep every existing import (ff.render_html_page, tests,
# nowcast facade) working unchanged. This block sits at the END of the
# module ON PURPOSE: rendering.py from-imports its shared constants and
# helpers back from this facade, so this module must be fully defined
# before rendering is imported. See forecast/README.md.
try:
    from .rendering import (             # noqa: F401,E402
        _landmarks_section_html, _landmarks_section_text,
        _render_accuracy_html, _render_accuracy_text,
        _render_cadence_ops_html, _render_cold_advisory_html,
        _render_cold_advisory_text, _render_day_cards_html,
        _render_flood_peaks_section, _render_flood_windows_html,
        _render_glossary_html, _render_how_flooding_html,
        _render_input_health_html, _render_input_health_text,
        _render_live_gauge_section, _render_lookahead_html,
        _render_lookahead_text, _render_low_tides_html,
        _render_low_tides_text, _render_more_details_html,
        _render_more_info_links_html, _render_oscillation_section,
        _render_pluvial_advisory_html, _render_rain_timing_html,
        _render_rain_timing_text, _render_recent_history_html,
        _render_recent_history_text, _render_reference_scale_html,
        _render_summary_html, _render_summary_text,
        _render_wind_adjustment_html, _spot_check_block_html,
        _spot_check_block_text, render_details_page, render_email,
        render_html_page, render_per_tide_page)
except ImportError:                      # run as a script from forecast/
    from rendering import (              # noqa: F401,E402
        _landmarks_section_html, _landmarks_section_text,
        _render_accuracy_html, _render_accuracy_text,
