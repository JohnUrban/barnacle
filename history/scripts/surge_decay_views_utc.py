...
MLLW_TO_NAVD88 = -2.82
# UTC rerun (audit 2026-09-24-a2 R1 follow-up): the original used naive
# station-local labels, whose DST transitions break exact hourly lags twice a
# year. The canonical UTC table has exact 1-h steps.
df = pd.read_parquet(ROOT / "history/data/sandy_hook_hourly_utc.parquet",
                     columns=["timestamp_utc", "surge_ft", "predicted_mllw", "observed_mllw"])
df = df.set_index("timestamp_utc").sort_index()
assert str(df.index.tz) == "UTC"
df = df[df.index >= pd.Timestamp("2005-01-01", tz="UTC")].asfreq("h")
s = df.surge_ft
means = {
...
keep = s.index >= pd.Timestamp("2006-01-01", tz="UTC")
S = s.values
